...
# Zero Trust 配置持久化文件（全局定义，确保所有函数可用）
...
EOF
    ...
    if ! command -v warp-cli &>/dev/null; then
        echo -e "${RED}WARP 安装失败${NC}"; exit 1
    fi
...
EOF
    ...
    # 创建 iptables 规则脚本
    # 注意：此处 IP 列表已去重，且注释行已移除（避免被 for 循环误读）
...
[Unit]
Description=WARP Transparent Proxy
...
test_connection() {
    ...
    if [ -n "$WARP_IP" ]; then
        ...
        echo -e "WARP 位置: ${GREEN}$(echo $WARP_INFO | grep -oP '"country":"\K[^"]+') - \
$(echo $WARP_INFO | grep -oP '"city":"\K[^"]+')${NC}"
    fi
}
...
do_install() {
    ...
    echo -e "\n${GREEN}╔════════════════════════════════════════════════════╗${NC}"
    echo -e "${GREEN}║            🎉 安装完成！流量已解锁 🎉               ║${NC}"
    echo -e "${GREEN}╚════════════════════════════════════════════════════╝${NC}"
    echo -e "\n${YELLOW}Google / Meta / PornHub 等流量现已自动通过 WARP！${NC}"
    echo -e "\n管理命令: ${CYAN}warp {status|start|stop|restart|test|ip}${NC}\n"
}
...
do_uninstall() {
    ...
    # 清理 Zero Trust 服务
    systemctl disable --now warp-zt-restore 2>/dev/null
...
do_status() {
    ...
    echo ""
    echo -e "${YELLOW}【Zero Trust 模式】${NC}"
    if [ -f "$ZT_CONFIG" ]; then
        source "$ZT_CONFIG"
        [ "$ZT_ENABLED" = "1" ] && \
            echo -e "${GREEN}已启用 (组织: $ZT_ORG)${NC}" || \
            echo -e "未启用"
    else
        echo -e "未配置"
    fi
...
do_show_ip() {
    ...
    echo -e "位置: $(echo $DIRECT_INFO | grep -oP '"country":"\K[^"]+') - \
$(echo $DIRECT_INFO | grep -oP '"city":"\K[^"]+')\n"
    ...
    if [ -n "$WARP_IP" ]; then
        ...
        echo -e "位置: $(echo $WARP_INFO | grep -oP '"country":"\K[^"]+') - \
$(echo $WARP_INFO | grep -oP '"city":"\K[^"]+')\n"
    else
...
save_zero_trust_config() {
    cat > "$ZT_CONFIG" << EOF
# WARP Zero Trust 配置（由脚本自动生成，勿手动修改）
ZT_ORG="$1"
ZT_MODE="proxy"
ZT_PORT="40000"
ZT_ENABLED="1"
...
do_setup_zero_trust() {
    ...
    echo -e "\n${YELLOW}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo -e "${YELLOW}请检查邮箱，获取 Cloudflare Zero Trust 验证码${NC}"
    echo -e "${YELLOW}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}\n"
    ...
    if [ "$test_result" = "200" ]; then
        ...
        echo -e "${YELLOW}连接测试返回: $test_result（可能需要稍等几秒）${NC}"
    fi
...
show_menu() {
    ...
    local zt_label=""
    if [ -f "$ZT_CONFIG" ]; then
        source "$ZT_CONFIG" 2>/dev/null
        [ "$ZT_ENABLED" = "1" ] && zt_label=" ${GREEN}[当前: ZT 模式 - $ZT_ORG]${NC}"
    fi

    echo -e "${YELLOW}请选择操作:${NC}\n"
    echo -e "  ${GREEN}1.${NC} 安装 WARP"
    echo -e "  ${GREEN}2.${NC} 卸载 WARP"
    echo -e "  ${GREEN}3.${NC} 查看状态 / IP / 连接测试"
    echo -e "  ${GREEN}4.${NC} 切换到 Zero Trust 模式${zt_label}"
